...
namespace Assets.Scripts.Managers
{
	public class DensityManager : IDisposable {
		...
		void DoModification(int3 pos, sbyte value){
			if(value == 0) return;
			var chunkPos = TerrainGeneration.Utils.WorldPosToChunkPos(pos);
			...
			if(densityData.fullChunks.Contains(chunkPos) || densityData.emptyChunks.Contains(chunkPos)){
				...
				unsafe{
					sbyte origValue = UnsafeUtility.ReadArrayElement<sbyte>((void*)data, TerrainGeneration.Utils.XyzToIndex(localPosInChunk, ChunkManager.chunkResolution));
					...
					UnsafeUtility.WriteArrayElement<sbyte>((void*)data, TerrainGeneration.Utils.XyzToIndex(localPosInChunk, ChunkManager.chunkResolution), value);
				}
			}
		}
