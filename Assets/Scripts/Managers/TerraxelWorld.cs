...
namespace Assets.Scripts.Managers
{
	...
	public class TerraxelWorld : MonoBehaviour
	{
		...
		public void Start(){
			...
			debugCanvas = UnityEngine.Resources.Load<GameObject>("Prefabs/TerraxelDebug");
			...
			chunkPrefab = UnityEngine.Resources.Load<GameObject>("Prefabs/Chunk");
			...
			if(drawChunkVariables.draw){
				...
				for(int i = 0; i < chunkDatas.Count; i++){
					...
					if(drawChunkVariables.dirMask){
						offset.y += 4f;
						Handles.Label(offset, TerrainGeneration.Utils.DirectionMaskToString(chunkDatas[i].dirMask));
					}
